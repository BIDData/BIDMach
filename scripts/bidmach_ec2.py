...
#            slave_address = get_dns_name(slave, opts.private_ips)
    ...
    amcommand="echo '%s' >  %s/conf/application.conf" % (akka_master_conf, bidmach_install_dir)
    ...
    for i in range(len(slave_names)):
        ...
        ascommand="echo '%s' >  %s/conf/application.conf" % (akka_slave_conf, bidmach_install_dir)
           ...
           opts.master_instance_type in EC2_INSTANCE_TYPES:
            if EC2_INSTANCE_TYPES[opts.instance_type] != \
               EC2_INSTANCE_TYPES[opts.master_instance_type]:
                print("Error: bidmach-ec2 currently does not support having a master and slaves "
