...
# Configures Spark and Hadoop installations on the Main and Subordinates
#
# What this script assumes:
#   (optional Spark and) Hadoop should be installed on the ec2 image
#   Hadoop should be configured to run on port 9000 on the main
...
def parse_args():
    parser = OptionParser(
        ...
        + "<action> can be: launch, destroy, login, stop, start, get-main, get-subordinates, reboot-subordinates")
    ...
    parser.add_option(
        "-n", "--node", type="int", 
        help="Number of subordinate to login to")
    parser.add_option(
        "-w", "--wait", type="int",
...
def get_or_make_group(conn, name, vpc_id):
    ...
    if len(group) > 0:
        ...
        return conn.create_security_group(name, "BIDMach EC2 group", vpc_id)
...
#Kylix
        subordinate_group.authorize(ip_protocol='tcp', from_port=50050, to_port=50060,
                               src_group=subordinate_group)
        subordinate_group.authorize(ip_protocol='udp', from_port=50050, to_port=50060,
                               src_group=subordinate_group)
        subordinate_group.authorize(ip_protocol='tcp', from_port=50050, to_port=50060,
                               src_group=main_group)
        subordinate_group.authorize(ip_protocol='udp', from_port=50050, to_port=50060,
                               src_group=main_group)
        main_group.authorize(ip_protocol='tcp', from_port=50050, to_port=50060,
                               src_group=subordinate_group)
        main_group.authorize(ip_protocol='udp', from_port=50050, to_port=50060,
                               src_group=subordinate_group)
...
def setup_cluster(conn, main_nodes, subordinate_nodes, opts, deploy_ssh_key, copyfiles):
    main = get_dns_name(main_nodes[0], False)
    local_main = get_dns_name(main_nodes[0], opts.private_ips)
    if deploy_ssh_key:
...
#            subordinate_address = get_dns_name(subordinate, opts.private_ips)
            subordinate_address = get_dns_name(subordinate, False)
            print(subordinate_address)
            ssh_write(subordinate_address, opts, ['tar', 'x'], dot_ssh_tar)

    print("configuring main %s" % main);

    subordinate_names = [get_dns_name(i, False) for i in subordinate_nodes];
    local_subordinate_names = [get_dns_name(i, opts.private_ips) for i in subordinate_nodes];
    subordinates_string = reduce(lambda x,y : x + "\n" + y, local_subordinate_names);
    
    bscommand="mkdir -p %s/conf; echo -e '%s' > %s/conf/subordinates" % (bidmach_install_dir, subordinates_string, bidmach_install_dir)
    ssh(main, opts, bscommand.encode('ascii','ignore'))

    bmcommand="echo -e '%s' > %s/conf/main" % (local_main, bidmach_install_dir)
    ssh(main, opts, bmcommand.encode('ascii','ignore'))

    hscommand="echo -e '%s' > %s/etc/hadoop/subordinates" % (subordinates_string, hadoop_install_dir)
    ssh(main, opts, hscommand.encode('ascii','ignore'))

    core_conf = core_site % local_main
    hccommand="echo '%s' >  %s/etc/hadoop/core-site.xml" % (core_conf, hadoop_install_dir)
    ssh(main, opts, hccommand.encode('ascii','ignore'))

    akka_main_conf = akka_conf % (local_main, local_main, local_main)
    amcommand="echo '%s' >  %s/conf/application.conf" % (akka_main_conf, bidmach_install_dir)
    ssh(main, opts,amcommand.encode('ascii','ignore'))

    ssh(main, opts, """rm -f ~/.ssh/known_hosts""")
    for i in range(len(subordinate_names)):
        subordinate = subordinate_names[i];
        local_subordinate = local_subordinate_names[i];
        ...
        ssh(subordinate, opts, hscommand.encode('ascii','ignore'))
        ssh(subordinate, opts, bscommand.encode('ascii','ignore'))
        ssh(subordinate, opts, bmcommand.encode('ascii','ignore'))
        akka_subordinate_conf = akka_conf % (local_subordinate, local_main, local_main)
        ascommand="echo '%s' >  %s/conf/application.conf" % (akka_subordinate_conf, bidmach_install_dir)
        ssh(subordinate, opts, ascommand.encode('ascii','ignore'))
...
def real_main():
           ...
           opts.main_instance_type in EC2_INSTANCE_TYPES:
            if EC2_INSTANCE_TYPES[opts.instance_type] != \
               EC2_INSTANCE_TYPES[opts.main_instance_type]:
                print("Error: bidmach-ec2 currently does not support having a main and subordinates "
    ...
    if action == "launch":
        ...
        setup_cluster(conn, main_nodes, subordinate_nodes, opts, True, True)
        ...
        if opts.node is not None:
            subordinate = subordinate_nodes[opts.node]
            if not subordinate.public_dns_name and not opts.private_ips:
                print("Subordinate has no public DNS name.  Maybe you meant to specify --private-ips?")
            else:
                subordinate = get_dns_name(subordinate, opts.private_ips)
                print("Logging into subordinate " + subordinate + "...")
                proxy_opt = []
                if opts.proxy_port is not None:
                    proxy_opt = ['-D', opts.proxy_port]
                subprocess.check_call(
                    ssh_command(opts) + proxy_opt + ['-t', '-t', "%s@%s" % (opts.user, subordinate)])
    ...
    elif action == "get-subordinates":
        (main_nodes, subordinate_nodes) = get_existing_cluster(conn, opts, cluster_name)
        for inst in subordinate_nodes:
            if not inst.public_dns_name and not opts.private_ips:
                print("Subordinate %s has no public DNS name.  Maybe you meant to specify --private-ips?" % inst)
            else:
        ...
        setup_cluster(conn, main_nodes, subordinate_nodes, opts, False, False)
