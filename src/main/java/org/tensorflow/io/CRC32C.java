...
public class CRC32C {
    ...
    private static final int [] table0_ = {
    ...
    private static final int [] table1_ = {
    ...
    private static final int [] table2_ = {
    ...
    private static final int [] table3_ = {
    ...
    public static void encodeFixed64(byte [] bytes, int offset, long value) {
    ...
    public static void encodeFixed32(byte [] bytes, int offset, int value) {
        bytes[offset] = (byte)(value & 0xff);
        bytes[offset+1] = (byte)((value >> 8) & 0xff);
        bytes[offset+2] = (byte)((value >> 16) & 0xff);
        bytes[offset+3] = (byte)((value >> 24) & 0xff);
    ...
    public static long decodeFixed64(byte [] bytes, int offset) {
        long b0 = ((long)bytes[offset]) & 0xffl;
        long b1 = ((long)bytes[offset+1]) & 0xffl;
        long b2 = ((long)bytes[offset+2]) & 0xffl;
        long b3 = ((long)bytes[offset+3]) & 0xffl;
        long b4 = ((long)bytes[offset+4]) & 0xffl;
        long b5 = ((long)bytes[offset+5]) & 0xffl;
        long b6 = ((long)bytes[offset+6]) & 0xffl;
        long b7 = ((long)bytes[offset+7]) & 0xffl;
        return (b7 << 56) | (b6 << 48) | (b5 << 40) | (b4 << 32) | (b3 << 24) | (b2 << 16) | (b1 << 8) | b0;
    }

    public static int decodeFixed32(byte [] bytes, int offset) {
        int b0 = ((int)bytes[offset]) & 0xff;
        int b1 = ((int)bytes[offset+1]) & 0xff;
        int b2 = ((int)bytes[offset+2]) & 0xff;
        int b3 = ((int)bytes[offset+3]) & 0xff;
        return (b3 << 24) | (b2 << 16) | (b1 << 8) | b0;
    }

    public static int unmask(int masked_crc) {
    ...
    public static int extend(int crc, byte [] buf, int offset, int size) {

        int e = offset + size;
        int p = offset;

        int l = crc ^ 0xffffffff;

        while (p + 4 <= e) {
            int c = l ^ decodeFixed32(buf, p);
            p += 4;                                            
        ...
        while (p < e) {
            int c = (l & 0xff) ^ buf[p];
            p += 1;
            l = table0_[c & 0xff] ^ (l >>> 8);
        }
        return l ^ 0xffffffff;
    }

    public static int getValue(byte [] data, int offset, int size) {
        return extend(0, data, offset, size);
    }
}
