...
object MNIST {
  def datasource(dir:String="/data/MNIST8M/parts/", nlast:Int = 80, n:Int = 1, i:Int = 0) = {
    ...
    val opts1 = new FilesSource.Options {  
    fnames = List(FilesSource.simpleEnum(dir+"/part%02d.imat.lz4", n, i));
    nstart = 0;
    nend = nlast;
    order = 0;
    batchSize = 10000;
    ...
    val opts2 = new SFilesSource.Options {  
    fnames = List(FilesSource.simpleEnum(dir+"/cats3col%02d.imat.lz4", n, i));
    nstart = opts1.nstart;
    nend = opts1.nend;
    order = opts1.order;
    ...
    new StackedDS(new FilesSource(opts1), new SFilesSource(opts2))
  }
...
//	  			println("")
  ...
  def junk:CSMat = {
	...
	def logisticModelPar(
	    nstart0:Int = FilesSource.encodeDate(2012,3,1,0),
			nend0:Int = FilesSource.encodeDate(2013,7,1,0),
			nuni0:Int = 50,
			nbi0:Int = 100,
			ntri0:Int = 200		
			) = {
...
//	  val expts = col(0.5)
  ...
  def twitterWords(
      nstart0:Int = FilesSource.encodeDate(2012,3,1,0), 
        nend0:Int = FilesSource.encodeDate(2012,12,1,0), 
        n:Int = 1, 
        i:Int = 0, 
        nfeats:Int = 100000) = {
    val opts = new SFilesSource.Options {  
        fnames = List(FilesSource.sampleFun(twitterFeatureDir + "unifeats%02d.lz4", n, i))
        fcounts = icol(nfeats)
        nstart = nstart0/n
        nend = nend0/n
        order = 1
  ...
  def twitterSmileyWords(
        ...
        nfeats:Int = 100000) = {
    val opts = new SFilesSource.Options {  
        fnames = List(FilesSource.sampleFun(twitterSmileyFeatureDir + "unifeats%02d.lz4", n, i))
        fcounts = icol(nfeats)
        nstart = nstart0/n
        nend = nend0/n
        order = 1
  ...
  def twitterNgrams(
    ...
    val opts = new SFilesSource.Options {  
        fnames = List(
                FilesSource.sampleFun(twitterFeatureDir + "unifeats%02d.lz4", n, i),
                FilesSource.sampleFun(twitterFeatureDir + "bifeats%02d.lz4", n, i),
                FilesSource.sampleFun(twitterFeatureDir + "trifeats%02d.lz4", n, i)
            )
  ...
  def twitterSmileyNgrams(
    ...
    val opts = new SFilesSource.Options {  
        fnames = List(
                FilesSource.sampleFun(twitterSmileyFeatureDir + "unifeats%02d.lz4", n, i),
                FilesSource.sampleFun(twitterSmileyFeatureDir + "bifeats%02d.lz4", n, i),
                FilesSource.sampleFun(twitterSmileyFeatureDir + "trifeats%02d.lz4", n, i)
            )
        fcounts = icol(nuni0*1000,nbi0*1000,ntri0*1000)
        nstart = nstart0/n
        nend = nend0/n 
  ...
  def testSources(nthreads:Int=4,ff:(Int,Int,Int,Int,Int)=>DataSource = twitterWords, nfeats:Int=100000):IMat = { 
    val nstart0 = FilesSource.encodeDate(2012,3,22,0)
    val nend0 = FilesSource.encodeDate(2013,7,1,0)
    var bytes = 0L
    var done = 0L
