...
class SFilesSourcev1(override val opts:SFilesSource.Opts = new SFilesSource.Options)(override implicit val ec:ExecutionContextExecutor) extends FilesSource(opts) {
 ...
 //       println("here %d %d %d %d %d" format (k, mat.nrows, mat.ncols, lims.length, j))
        while (k < mat.nrows && mat.data(k) == irow && mat.data(k+mrows) < lims(j)) {
          if (xoff + k >= omat.ir.length) {
            throw new RuntimeException("SFilesSource index out of range. Try increasing opts.eltsPerSample")
          }
...
class SFilesSource(override val opts:SFilesSource.Opts = new SFilesSource.Options)(override implicit val ec:ExecutionContextExecutor) extends FilesSource(opts) {
...
object SFilesSource {
  trait Opts extends FilesSource.Opts {
  	var fcounts:IMat = null
  }
