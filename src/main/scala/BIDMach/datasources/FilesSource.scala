...
class FilesSource(override val opts:FilesSource.Opts = new FilesSource.Options)(implicit val ec:ExecutionContextExecutor) extends DataSource(opts) { 
  var sizeMargin = 0f
  var blockSize = 0 
  ...
  def reset = {
    ...
    if (opts.order == 0) {
      ...
      permfn = (n:Int) => {                                                    // Stripe reads across disks (different days)
        val (yy, mm, dd, hh) = FilesSource.decodeDate(n)
        val hhdd = hh + 24 * (dd - 1)
        FilesSource.encodeDate(yy, mm, hhdd % 31 + 1, hhdd / 31)
      } 
    }
    rowno = 0;
    fileno = nstart;
  ...
  def init = {
    ...
    for (i <- 0 until fnames.size) {
      ...
      omats(i) = mm match {
        case mf:FMat => FMat.newOrCheckFMat(nr, nc, null, GUID, i, ((nr*1L) << 32) + nc, "FilesSource_FMat".##);
        case mi:IMat => IMat.newOrCheckIMat(nr, nc, null, GUID, i, ((nr*1L) << 32) + nc, "FilesSource_IMat".##);
        case md:DMat => DMat.newOrCheckDMat(nr, nc, null, GUID, i, ((nr*1L) << 32) + nc, "FilesSource_DMat".##);
        case ms:SMat => SMat.newOrCheckSMat(nr, nc, nc * opts.eltsPerSample, null, GUID, i, ((nr*1L) << 32) + nc, "FilesSource_SMat".##);
      }
    } 
  }
  ...
  def checkCaches(nr:Int, nc:Int, out:Mat, GUID:Long, i:Int):Mat = {
    if (nr == out.nrows && nc == out.ncols) {
      out 
    	...
    	out match {
    	case a:FMat => FMat.newOrCheckFMat(nr, nc, null, GUID, i, ((nr*1L) << 32) + nc, "FilesSource_FMat".##);
    	case a:IMat => IMat.newOrCheckIMat(nr, nc, null, GUID, i, ((nr*1L) << 32) + nc, "FilesSource_IMat".##);
    	case a:DMat => DMat.newOrCheckDMat(nr, nc, null, GUID, i, ((nr*1L) << 32) + nc, "FilesSource_DMat".##);
    	case a:SMat => SMat.newOrCheckSMat(nr, nc, a.nnz, null, GUID, i, ((nr*1L) << 32) + nc, "FilesSource_SMat".##);
    	}
    }
  }
...
object FilesSource {
  ...
  def apply(opts:FilesSource.Opts, nthreads:Int):FilesSource = {
    implicit val ec = threadPool(nthreads);
    new FilesSource(opts);
  }
  
  def apply(opts:FilesSource.Opts):FilesSource = apply(opts, 4);
  
  def apply(fname:String, opts:FilesSource.Opts, nthreads:Int):FilesSource = {
    opts.fnames = List(simpleEnum(fname, 1, 0));
    implicit val ec = threadPool(nthreads);
    new FilesSource(opts);
  }
  
  def apply(fname:String, opts:FilesSource.Opts):FilesSource = apply(fname, opts, 4);
  
  def apply(fname:String):FilesSource = apply(fname, new FilesSource.Options, 4);
  
  def apply(fn1:String, fn2:String, opts:FilesSource.Opts, nthreads:Int) = {
    opts.fnames = List(simpleEnum(fn1, 1, 0), simpleEnum(fn2, 1, 0));
    implicit val ec = threadPool(nthreads);
    new FilesSource(opts);
  }
  
  def apply(fn1:String, fn2:String, opts:FilesSource.Opts):FilesSource = apply(fn1, fn2, opts, 4);
