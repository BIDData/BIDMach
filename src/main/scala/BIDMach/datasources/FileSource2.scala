...
import BIDMat.{Mat,SBMat,CMat,CSMat,DMat,FMat,IMat,HMat,GMat,GIMat,GSMat,SMat,SDMat,ND}
...
class FileSource2(override val opts:FileSource.Opts = new FileSource.Options) extends DataSource(opts) {
	var sizeMargin = 0f;
	var blockSize = 0;
	...
	def init = {
		...
		for (i <- 0 until fnames.size) {
			...
			omats(i) = mm match {
			case mf:FMat => FMat.newOrCheckFMat(dims, null, GUID, i, hashdims, "FileSource2_FMat".##);
			case mi:IMat => IMat.newOrCheckIMat(dims, null, GUID, i, hashdims, "FileSource2_IMat".##);
			case md:DMat => DMat.newOrCheckDMat(dims, null, GUID, i, hashdims, "FileSource2_DMat".##);
			case ms:SMat => SMat.newOrCheckSMat(dims(0), dims(1), dims(1) * opts.eltsPerSample, null, GUID, i, ((dims(0)*1L) << 32) + dims(1), "FileSource2_SMat".##);
			}
		}
	}
	...
	def reset = {
		...
		if (opts.order == 0) {
			...
			permfn = (n:Int) => {                                                    // Stripe reads across disks (different days)
				val (yy, mm, dd, hh) = FileSource2.decodeDate(n);
				val hhdd = hh + 24 * (dd - 1);
				FileSource2.encodeDate(yy, mm, hhdd % 31 + 1, hhdd / 31);
			}
		}
	...
	def next:Array[Mat] = {
				...
				for (i <- 0 until fnames.size) {
					...
					if (matq0.asInstanceOf[AnyRef] == null) {
					  ...
					  matqnc = matq0.ncols;
						todo = math.min(blockSize, matqnc - colno);
						todo1 = if (todo == blockSize || (fileno+1) >= nend) 0 else math.min(blockSize - todo, matq1.ncols);
						val off = Mat.oneBased;
						val dims = omats(i).dims.copy;
						dims(dims.length-1) = todo + todo1;
						omats(i) = checkCaches(dims, omats(i), GUID, i);
						if (todo1 == 0) {
							matq0.colslice(colno, colno+todo, omats(i), 0);
						} else {
							omats(i) <-- (matq0.colslice(colno, colno+todo, null, 0) \ matq1.colslice(0, todo1, null, 0));
						}
...
//				println("hash %f" format sum(sum(omats(0))).dv);
		...
		def checkCaches(dims:IMat, out:Mat, GUID:Long, i:Int):Mat = {
				if (ND.compareDims(dims.data, out.dims.data)) {
					...
					val hashdims = ND.hashInts(dims.data);
					out match {
					case a:FMat => FMat.newOrCheckFMat(dims, null, GUID, i, hashdims, "FileSource2_FMat".##);
					case a:IMat => IMat.newOrCheckIMat(dims, null, GUID, i, hashdims, "FileSource2_IMat".##);
					case a:DMat => DMat.newOrCheckDMat(dims, null, GUID, i, hashdims, "FileSource2_DMat".##);
					case a:SMat => SMat.newOrCheckSMat(dims(0), dims(1), a.nnz, null, GUID, i, hashdims, "FileSource2_SMat".##);
					}
				}
		}
...
object FileSource2 {
	...
	def apply(opts:FileSource.Opts, nthreads:Int):FileSource2 = {
			implicit val ec = threadPool(nthreads);
			new FileSource2(opts);
	}

	def apply(opts:FileSource.Opts):FileSource2 = apply(opts, 4);

	def apply(fname:String, opts:FileSource.Opts, nthreads:Int):FileSource2 = {
			opts.fnames = List(simpleEnum(fname, 1, 0));
			implicit val ec = threadPool(nthreads);
			new FileSource2(opts);
	}

	def apply(fname:String, opts:FileSource.Opts):FileSource2 = apply(fname, opts, 4);

	def apply(fname:String):FileSource2 = apply(fname, new FileSource.Options, 4);
	...
	def apply(fn1:String, fn2:String, opts:FileSource.Opts):FileSource2 = apply(fn1, fn2, opts, 4);
