...
import BIDMat.{Mat,SBMat,CMat,DMat,FMat,FND,IMat,HMat,GDMat,GMat,GIMat,GSDMat,GSMat,GND,ND,SMat,SDMat}
...
abstract class FactorModel(override val opts:FactorModel.Opts) extends Model(opts) {
  ...
  def dobatch(gmats:Array[ND], ipass:Int, i:Long) = {
    val sdata = gmats(0).asMat;
    val user = FactorModel.reuseuser(gmats(0).asMat, opts.dim, opts.initUval)
  ...
  def evalbatch(mats:Array[ND], ipass:Int, here:Long):FMat = {
    ...
    if (gmats.length > 2) {
    	evalfun(sdata, user, gmats(2).asMat, ipass, here);
    } else {
